"""CloudS2: Sentinel 2's satellite rgb [band 4-3-2] images augmented with synthetic clouds.
...
class CloudS2(Dataset):
    def __init__(self, 
        ...
        assert task in ["classification", "segmentation"], f"{task} is not valid."
    ...
    def get_target(self, use_threshold=True):
        if self.task == "classification":
            target = torch.empty(self.TOTAL_FRAMES)
