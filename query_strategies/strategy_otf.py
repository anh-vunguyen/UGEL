...
class StrategyOTF:
    ...
    def _train(self, epoch, loader_tr, optimizer):
        ...
        for batch_idx, (x, y, idxs) in enumerate(loader_tr):
            ...
            if self.args['net_type'] in ['pretrainedS2_resnet50' , 'pretrainedL9_resnet50', "vanilla_resnet50"]:
                out = self.reg(x)
            elif self.args['net_type'] in ["bayesian_net", "bayesian_resnet18", "mcdropout_math_regressor", "mcdropout_math_regressor_SS", "bayesian_mobilenetv3", "bayesian_mobilenetv4"]: # NOTE: 08/10/2025 - # NOTE: 05/05/2025
                out = self.reg(x, k=1)
                out = out.squeeze(-1)
            elif self.args['net_type'] in ["gaussian_resnet18", "gaussian_net_semi_supervised"]: # NOTE: 12/04/2025
                out = self.reg(x)
                mu, sigma = torch.chunk(out, 2, dim=-1)
                mu = mu.squeeze(-1)
                sigma = sigma.squeeze(-1)
            elif self.args['net_type'] in ["evidential_resnet18", "evidential_net_semi_supervised", "evidential_math_regressor", "evidential_math_regressor_SS"]: # NOTE: 12/04/2025
                out = self.reg(x)
            ...
            elif self.args['net_type'] in ["evidential_resnet18", "evidential_net_semi_supervised", "evidential_math_regressor", "evidential_math_regressor_SS"]: # NOTE: 15/04/2025
    ...
    def train(self):
        ...
        if self.args['net_type'] == 'pretrained': 
            self.reg = self.args['pretrained_resnet']().cuda() # Regressor
        elif self.args['net_type'] == 'pretrained_resnet18':
            self.reg = self.args['pretrained_resnet18']().cuda() # Regressor
        ...
        elif self.args['net_type'] == 'beta_net_semi_supervised_2': # NOTE: 22/05/2025
            self.reg = self.args['beta_net_semi_supervised_2']().cuda() # NOTE: 22/05/2025
        elif self.args['net_type'] == "resnet18_semi_supervised": # NOTE: 24/07/2025
            self.reg = self.args['resnet18_semi_supervised']().cuda()
        elif self.args['net_type'] == "bayesian_mobilenetv3": # NOTE: 09/10/2025
            self.reg = self.args['bayesian_mobilenetv3']().cuda()
        elif self.args['net_type'] == "mobilenetv3": # NOTE: 09/10/2025
            self.reg = self.args['mobilenetv3']().cuda()
        elif self.args['net_type'] == "beta_mobilenetv3": # NOTE: 09/10/2025
            self.reg = self.args['beta_mobilenetv3']().cuda()
        elif self.args['net_type'] == "beta_mobilenetv3_semi_supervised": # NOTE: 09/10/2025
        ...
        elif self.args['net_type'] == "beta_mobilenetv4_semi_supervised": # NOTE: 09/10/2025
            self.reg = self.args['beta_mobilenetv4_semi_supervised']().cuda()             
        else: 
            self.reg =  self.net.apply(weight_reset).cuda() # Regressor
            
        #  
        if self.args['net_type'] in ['resnet18_semi_supervised', 'bayesian_net_semi_supervised', 'bayesian_resnet18_cloudscout', "gaussian_net_semi_supervised", "evidential_net_semi_supervised", "beta_net_semi_supervised", "beta_mobilenetv3_semi_supervised", "beta_mobilenetv4_semi_supervised"]:
            ...
            if self.args['data'] == "CloudL9":
                ...
                loader_tr = DataLoader(self.handler(dataset), shuffle=True, **self.args['loader_tr_args'])
            elif self.args['data'] == "CloudL8_128": # NOTE: 15 Mar 2025
                dataset = CloudL9(idxs_train, self.args["path"], img_shape=(3, 128, 128))
                loader_tr = DataLoader(self.handler(dataset), shuffle=True, **self.args['loader_tr_args'])
            elif self.args['data'] == "CloudSEN12_128": # NOTE: 26 Jun 2025
                dataset = CloudL9(idxs_train, self.args["path"], img_shape=(3, 128, 128))
                loader_tr = DataLoader(self.handler(dataset), shuffle=True, **self.args['loader_tr_args'])
            elif self.args['data'] == "LandCoverAI_128": # NOTE: 26 Jun 2025
                dataset = CloudL9(idxs_train, self.args["path"], img_shape=(3, 128, 128))
                loader_tr = DataLoader(self.handler(dataset), shuffle=True, **self.args['loader_tr_args'])
            else:
    ...
    def predict(self, test_dataset, for_testing=True):
        ...
        with torch.no_grad():
            for x, y, idxs in loader_te:
                ...
                if self.args['net_type'] in ['pretrainedS2_resnet50' , 'pretrainedL9_resnet50']:
                ...
                elif self.args['net_type'] in ["bayesian_net", "bayesian_resnet18", "bayesian_net_semi_supervised", "bayesian_resnet18_cloudscout", "bayesian_mobilenetv3", "bayesian_mobilenetv4"]:
                    out = self.reg(x, k=10)
                    out = out.mean(dim=1)
                    outs[idxs] = out.squeeze(-1).cpu()
                    ...
                    beta = beta.squeeze(-1)
                    outs[idxs] = mu.cpu()
                elif self.args['net_type'] in ['beta_resnet18', "beta_net_semi_supervised", "beta_math_regressor_SS", "beta_mobilenetv3", "beta_mobilenetv3_semi_supervised", "beta_mobilenetv4_semi_supervised"]:
                    out = self.reg(x)
                    mu, nu = torch.chunk(out, 2, dim=-1)
                    mu = mu.squeeze(-1)
                    nu = nu.squeeze(-1)
